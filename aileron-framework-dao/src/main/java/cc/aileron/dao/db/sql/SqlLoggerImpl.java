...
public class SqlLoggerImpl implements SqlLogger
{
    ...
    @Override
    public void output(final SqlLoggerParameter p)
    {
        ...
        else
        {
            ...
            for (final Object object : p.arguments)
            {
                if (matcher.find())
                {
                    ...
                    {
                        final Enum<? extends NumberGetAccessor> value = Cast.cast(object);
     ...
     */
    @Inject
    public SqlLoggerImpl()
    {
        this.logger = LoggerFactory.getLogger(SqlLogger.class);
        this.isEnabled = this.logger.isDebugEnabled();
    }
